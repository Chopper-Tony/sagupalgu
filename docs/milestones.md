...
| M143: 마켓 고도화 | ✅ 완료 | 상품 상세 페이지(#/market/{id}, 이미지 갤러리+플랫폼 게시 링크 바로가기) ✅, 검색+필터(키워드 ILIKE+태그 매칭+가격 범위, 디바운스 300ms) ✅, 구매 문의(Discord 웹훅 전송, 문의 폼) ✅, MarketDetailPage+CSS 신규 ✅, search_completed()+get_completed_by_id() 메서드 ✅, 723 테스트 통과 ✅ |
| M136: 프론트 UX 개선 | ✅ 완료 | ErrorCard 오류 코드 details 접기 ✅, ProductConfirmationCard 필드 검증+후보 없음 메시지 ✅, DraftCard 게시 준비 로딩 상태 ✅, ChatComposer textarea 자동 높이+disabled 플레이스홀더 ✅, Progress 메시지 구체화 ✅, OptimizationSuggestionCard 새로 시작하기 확인 ✅, 723 테스트 통과 ✅ |
| M137: 자체 마켓 프로토타입 | ✅ 완료 | GET /api/v1/market(공개 목록, 인증 불필요) ✅, SessionRepository.list_completed() ✅, MarketPage 카드 그리드(다크 테마, 반응형) ✅, 해시 라우팅(#/market) ✅, 723 테스트 통과 ✅ |
| M135: 게시 UX 통합 + RLS + 문서 | ✅ 완료 | 세션 ID 수동 복사 제거(publish_bridge.js+FETCH_AND_PUBLISH로 웹앱→background 자동 통신) ✅, PublishResultCard "자동 게시" 버튼(window.postMessage) ✅, RLS 마이그레이션 파일화(003_rls_policies.sql) ✅, EBS 19GB 증설+Docker 정상 재빌드 복구 ✅, 중고나라 자동 게시 버튼 E2E 성공 ✅, 문서 정합화 ✅, 723 테스트 통과 ✅ |
| M134: 번개장터 Content Script | ✅ 완료 | bunjang_publish.js Content Script 구현 ✅, EXTENSION_ONLY_PLATFORMS에 bunjang 추가(서버 Playwright 완전 제거) ✅, CDP 이미지 업로드 재사용 ✅, 팝업 번개장터 자동 게시 버튼 ✅, 테스트 정합성 수정 ✅, 723 테스트 통과 ✅ |
| M133: 부분 성공 처리 + 중고나라 CDP 이미지 업로드 | ✅ 완료 | 게시 부분 성공 로직(1개 성공→completed, 전부 실패→publishing_failed) ✅, ChatWindow ProgressCard/ErrorCard case 누락 수정 ✅, 중고나라 CloudFront 403 감지+access_blocked 에러 분류 ✅, langgraph-prebuilt 버전 고정(1.0.7, ExecutionInfo import 해결) ✅, 크롬 익스텐션 중고나라 자동 게시(Content Script+background CORS 우회+이미지 data URL 변환) ✅, GET /publish-data·POST /extension-publish-result API ✅, PublishResultCard 서버 차단 안내+세션 ID 복사 ✅, 723 테스트 통과 ✅ |
| M132: EC2 배포 + 번개장터 자동 게시 E2E 성공 | ✅ 완료 | EC2(us-east-1 t3.micro) 배포 ✅, Docker Compose(backend+frontend) 서비스 구동 ✅, API baseURL 상대 경로 전환(`/api/v1`) ✅, nginx CORS preflight 처리(OPTIONS 204) ✅, nginx 업로드 크기 50M ✅, Dockerfile legacy_spikes 복사 ✅, .dockerignore legacy_spikes 허용 ✅, Caddyfile git 추가 ✅, CI deploy EC2_HOST 미설정 시 스킵 ✅, ENVIRONMENT=dev 전환 ✅, 번개장터 headless 자동 게시 성공 ✅ |
| M131: 크롬 익스텐션 플랫폼 연동 | ✅ 완료 | Manifest V3 익스텐션 구현(popup+background+cookies.js) ✅, connect_token API(1회용→플랫폼별 재사용, 10분 TTL) ✅, 쿠키 수집(endsWith 도메인 필터링+sameSite 변환) ✅, 사전 체크(쿠키 0개→전송 차단) ✅, user별 세션 분리 저장(sessions/{user_id}/) ✅, 공용 경로 동기(legacy publisher 호환) ✅, 세션 검증(쿠키 기반, IP 불일치 대응) ✅, 프론트 SessionSidebar connect token 발급 UI ✅, CSP 준수(인라인 onclick→addEventListener) ✅, DraftCard 이미지 섹션 제거 ✅ |
| M130: 배포 문서 + 운영 안전장치 + Worker 정리 | ✅ 완료 | deployment.md 전면 재작성(장애 대응·스케일 한계·로그/모니터링) ✅, docker-compose.prod.yml worker 메모리 제한(1536M)+MAX_CONCURRENT_BROWSERS=1 ✅, .env.example S3/도메인 설정 추가 ✅, check_prod_readiness.py DOMAIN_NAME 검증 ✅, _active_jobs 완전 제거(단일 진실 _active_tasks) ✅ |
| M125: 테스트 정합성 복구 | ✅ 완료 | test_e2e_recovery 14개 green(PUBLISH_USE_QUEUE=False fixture로 동기 경로 격리, DB 의존성 차단) ✅, test_s3_auxiliary 6개 green(boto3 미설치 환경 sys.modules mock 주입) ✅, 실패 테스트 0건 달성 ✅, 714 테스트 통과 ✅ |
| M123: 배포 블로커 제거 (CTO v7/v8/v9 P0 대응) | ✅ 완료 | Admin API `X-Admin-Key` 인증 ✅, Worker task set 추적+graceful shutdown ✅, `_active_jobs` 버그 제거 ✅, `_semaphore._value` 제거 ✅, `on_event`→`lifespan` 전환 ✅, `RUN_PUBLISH_WORKER` 플래그 ✅, admin_router repo private 접근 제거 ✅, 702 테스트 통과 ✅ |
| M122: 게시 링크 정합성 + UX 개선 | ✅ 완료 | publish_worker 결과 누적 저장 ✅, 번개장터 리다이렉트 폴링 30초 ✅, 중고나라 completeSeq URL 파싱 ✅, 프론트 확정 메시지/스크롤/게시결과 카드 개선 ✅ |
...
| M123: 배포 블로커 제거 (CTO v7/v8/v9 P0 대응) | ✅ 완료 | Admin API `X-Admin-Key` 인증 추가(전 엔드포인트 Depends) ✅, PublishWorker `_active_jobs` 유령 버그 제거 ✅, `_semaphore._value` 직접 접근 제거→task set 크기 판단 ✅, `asyncio.create_task` fire-and-forget→task set 추적+done_callback ✅, `stop()` graceful shutdown(in-flight task drain) ✅, `on_event`→`lifespan` context manager 전환(deprecation 해소) ✅, `RUN_PUBLISH_WORKER` 플래그(API/Worker 역할 분리) ✅, `admin_router` repo._get_client() 직접 접근 제거→`list_jobs`/`reset_to_pending` 정식 메서드 ✅, config.py `ADMIN_API_KEY`·`RUN_PUBLISH_WORKER` 추가 ✅, 테스트 8개 추가 ✅, 702 테스트 통과(warnings 22→2) ✅ |
| M122: 게시 링크 정합성 + UX 개선 | ✅ 완료 | publish_worker 결과 누적 저장(먼저 완료된 플랫폼 URL 소실 방지) ✅, 번개장터 PatchedBunjangPublisher 리다이렉트 폴링 30초(3초 간격) ✅, 중고나라 completeSeq URL 파싱 ✅, URL 키 이름 통일(listing_url→external_url) ✅, 프론트 확정 메시지 카테고리 포함 ✅, 스크롤 튐 방지(300ms debounce) ✅, PublishResultCard 성공 시 항상 링크 표시 ✅ |
...
| 마일스톤 | 상태 | 주요 변경 |
|---|---|---|
| M144: 거래 루프 Phase 1 | ✅ 완료 | 판매 상태 관리(available/reserved/sold, 전이 규칙+race condition 방어), 문의 이력 DB 저장(inquiries 테이블+InquiryRepository+자동 상태 전이), 판매자 대시보드 코어(#/my-listings, 상태 변경+문의 뱃지), dev bypass prod 차단 안전장치, api.ts dev header 자동 주입 |
| M145: 마켓 Phase 2 | ✅ 완료 | 재등록 원클릭(POST /my-listings/{id}/relist, 세션 복제+가격 조정), 카테고리 필터(confirmed_product.category 기반 7종), 문의 응대 코파일럿(LLM 응답 초안+goal별 fallback 템플릿 3종+문의 유형 자동 감지) |
| M146: 마켓 Phase 3 | ✅ 완료 | 자동 가격 조정 제안(3일/7일/14일 무문의 기반 copilot_suggestions), Publish Visibility(외부 플랫폼 게시 상태+URL 대시보드 표시), 정렬 옵션(최신순/가격순), CLAUDE.md 148줄→102줄 축소 |
| M147: 마켓 Phase 4 | ✅ 완료 | 판매 성과 대시보드(view_count 자동 증가+조회수/시세 대비 위치), 찜하기(useWishlist 로컬 스토리지+하트 버튼+찜 필터), 판매자 프로필(GET /sellers/{user_id}/profile+MarketDetailPage 카드) |
| M148: 마켓 테스트 보강 | ✅ 완료 | 56개 unit 테스트 추가(525→581), test_market_helpers(35개: sale_status 전이+copilot suggestions+publish results+goal_strategy), test_market_api(21개: 공개 API+판매자 API 통합), /{session_id} 라우트 순서 bugfix |
| M149: 최근 본 상품 + 문서 | ✅ 완료 | useRecentlyViewed 훅(로컬 스토리지, 최근 5개), MarketDetailPage 자동 기록, MarketPage 하단 "최근 본 상품" 섹션, 문서 정합화 |

## UX + 인프라 고도화 (M150~M157)

| 마일스톤 | 상태 | 주요 변경 |
|---|---|---|
| M150: 챗봇 환각 방지 + rate limit (#173/#176) | ✅ 완료 | 구매자용 AI 상품 챗봇 환각 방지 강화(상품 설명 vs 일반 스펙 분기·추정 금지 항목 5종), fallback 사유 구분(no_context/error), OpenAI rate limit 대응 |
| M151: 이메일 알림 Gmail SMTP (#171) | ✅ 완료 | 구매 문의 시 Discord 웹훅 + Gmail SMTP 병렬 알림, `_send_inquiry_email()` 함수 신설(smtp.gmail.com:465, SMTP_SSL), 각 채널 독립 실패 처리 |
| M152: 라이트/다크 테마 + 플랫폼 카드 개편 (#177/#178) | ✅ 완료 | ThemeToggle 컴포넌트, CSS 변수 기반 테마 시스템(`--btn-padding`·`--text-primary` 등 공통 토큰), 플랫폼 연결 카드 제거 + 토큰 자동 전달, 익스텐션 라이트모드 |
| M153: UI 폴리시 (#179~#184, #186~#193) | ✅ 완료 | 테마 토글 버그 수정, 하드코딩 색상 전수 교체(CSS 변수화), 토글/버튼 크기 전역 통일(padding 6px 14px, radius 8px, font 13px), 사이드바 nav padding 축소, 하단 라인 정렬, 게시 버튼 인라인 |
| M154: 모바일 반응형 (#185, #188, #191, #192) | ✅ 완료 | 모바일 감지(`window.innerWidth <= 768`), 사이드바 숨김, 세션 자동 생성 + 업로드, 짧은 placeholder, 네비 통일 |
| M155: 모바일 게시 분기 (#196, #197, #199) | ✅ 완료 | 판매글 전체 복사 버튼 + 플랫폼 `products/new` 직접 올리기 링크, PublishResultCard 모바일 분기(`isMobile` state), 데스크톱 익스텐션 자동 게시 유지 |
| M156: 서울 리전 Elastic IP 고정 (#195) | ✅ 완료 | us-east-1 → ap-northeast-2 이전, Elastic IP `43.201.188.57` 할당(재시작해도 IP 불변), 익스텐션 URL 하드코딩 해소, 키 파일 `sagupalgu-seoul-key.pem`, 인스턴스 t3.medium(2vCPU/4GB) |
| M157: CTO P1 단일화 리팩토링 (#200/#201) | ✅ 완료 | `app/domain/publish_policy.py` 게시 정책 단일 원천(타임아웃·재시도·에러 분류·플랫폼 capability 통합), `frontend/src/hooks/useSessionActions.ts` 훅 분리(App.tsx에서 10종 액션 비즈니스 로직 추출 — createActionHandler 패턴) |
| M158: 문서 전면 최신화 (#202~#204) | ✅ 완료 | CLAUDE.md 서울 리전 + AI 챗봇 + 모바일 + CTO P1 반영, `docs/deployment.md`·`architecture.md`·`demo-guide` 최신화 |
