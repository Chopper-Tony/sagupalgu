"""
경로: sagupalgu/save_sessions.py

중고나라 / 번개장터 세션 저장 스크립트
- 최초 1회만 실행하면 됨
- 로그인 후 세션(쿠키)을 JSON 파일로 저장
- 이후 자동화에서는 세션 파일 로드해서 로그인 스킵
"""
import asyncio
import sys
...
async def save_joongna_session():
    ...
    async with async_playwright() as p:
        ...
        await page.goto("https://web.joongna.com/signin?type=default", wait_until="domcontentloaded")
        await asyncio.sleep(2)
